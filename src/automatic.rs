...
fn evidence_fingerprint(prompt: &crate::ai::Prompt) -> Vec<(String, String)> {
    let mut evidence: Vec<_> = prompt
        .evidence
        .iter()
...
fn supported(answers: &Value, field: &str, threshold: f64) -> bool {
    serde_json::from_value::<Choice>(answers[field].clone())
        .is_ok_and(|answer| answer.supports("supported", threshold))
}
...
async fn advance_locked(db: &Db, config: &Config, binary: &str, max_passes: u32) -> Result<bool> {
    ...
    let outcome: Result<()> = async {
        match phase.as_str() {
            "assessing" => {
                let proposals = assess(db, config, binary).await?;
            ...
            "types" => {
                let proposals: Vec<String> = serde_json::from_str(&cycle.get::<String, _>("type_results"))?;
                if !proposals.is_empty() {
                    let operation = match cycle.get::<Option<String>, _>("type_operation") {
                        Some(operation) => Some(operation),
                        None => match crate::types::preview_many(db, config, &proposals).await {
                            Ok(preview) => {
            ...
            "reanalysis" => {
                ...
                if pass + 1 < limit && config.ai.configured() {
                    ...
                    for function in functions {
                        ...
                        if previous.as_ref().is_some_and(|old| evidence_fingerprint(old) == evidence_fingerprint(&prompt)) {
                            sqlx::query("UPDATE results SET automation_json=json_set(automation_json,'$.reason','Stopped automatically: expanded context contains no new evidence') WHERE id=(SELECT current_result_id FROM functions WHERE id=?)").bind(&function).execute(&db.pool).await?;
                            continue;
                        }
                ...
                if !prompts.is_empty() {
                    ...
                    tx.commit().await?;
                    db.event(binary, "info", &format!("Automatic recovery widened context for {} unresolved functions in pass {} of {limit}.", prompts.len(), pass + 2)).await?;
                } else {
...
async fn assess(db: &Db, config: &Config, binary: &str) -> Result<Vec<String>> {
    ...
    let threshold = config
        ...
        .decisions
        .as_ref()
        .map_or(0.95, |d| d.confidence_threshold);
    ...
    for row in rows {
        ...
        let name = if valid && supported(&answers["answers"], "name", threshold) {
            "accepted"
        } else {
            "deferred"
        };
        let summary = if valid && supported(&answers["answers"], "summary", threshold) {
            "accepted"
        } else {
            "deferred"
        };
        let types = if analysis.type_plan.is_empty() {
            "none"
        } else if valid && supported(&answers["answers"], "types", threshold) {
            "validated"
        } else {
            "deferred"
        };
        let reason = "Fields require independent evidence support; uncertain fields are deferred automatically";
        ...
        let audit =
            json!({"name":name_status,"summary":summary_status,"types":types,"reason":reason});
    ...
    plans.clear();
    let ready = sqlx::query("SELECT r.id,r.raw_json FROM functions f JOIN results r ON r.id=f.current_result_id WHERE f.binary_id=? AND r.stale=0 AND json_extract(r.automation_json,'$.types')='validated'")
        .bind(binary).fetch_all(&db.pool).await?;
    for row in ready {
        let analysis: Analysis = serde_json::from_str(&row.get::<String, _>("raw_json"))?;
        plans.push((row.get("id"), analysis.type_plan));
    }
...
mod tests {
    ...
    async fn independent_fields_defer_without_manual_review_and_resume_idempotently() {
        ...
        let response = json!({"answers":{
            "name":{"type":"choice","choice":"supported","confidence":1.0,"probabilities":{"supported":1.0}},
        ...
        assert!(assess(&db, &config, "b").await.unwrap().is_empty());
    ...
    #[tokio::test]
    async fn wider_context_adds_neighbor_evidence_and_stops_at_total_pass_limit() {
        ...
        let second = ai.recovery_prompt(&db, "b:1000", "map", 1).await.unwrap();
        let third = ai.recovery_prompt(&db, "b:1000", "map", 2).await.unwrap();
        assert!(second.config.max_input_bytes > initial.config.max_input_bytes);
        assert!(third.config.max_input_bytes >= second.config.max_input_bytes);
        assert!(second.evidence.len() > initial.evidence.len());
        assert_eq!(evidence_fingerprint(&second), evidence_fingerprint(&third));
        for prompt in [&second, &third] {
            assert!(
                serde_json::to_vec(&prompt.messages).unwrap().len()
                    <= prompt.config.max_input_bytes
            );
            let context: Value =
                serde_json::from_str(prompt.messages[1]["content"].as_str().unwrap()).unwrap();
            assert_eq!(context["recovery_feedback"].as_array().unwrap().len(), 1);
            assert!(!context["neighbors"].as_array().unwrap().is_empty());
        }
