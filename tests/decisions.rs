...
async fn preprocessing_generation_and_verification_preserve_review_and_accounting() {
    ...
    assert!((o.cost_usd.unwrap() - 0.0000084).abs() < 1e-10);
    assert_eq!(o.unreported_cost_requests, 1);
...
async fn missing_confidence_cannot_defer_and_double_finish_is_idempotent() {
    ...
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM jobs WHERE stage='map'")
        ...
        .unwrap();
    assert_eq!(count, 1);
    assert!((f.db.overview("b").await.unwrap().cost_usd.unwrap() - 0.0000042).abs() < 1e-10);
}
#[tokio::test]
...
#[tokio::test]
async fn pinned_decision_provider_survives_configuration_changes() {
